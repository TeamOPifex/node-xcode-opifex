...
pbxProject.prototype.addSourceFileToGroup = function (path, group, opt) {
    var file;
    var groupKey = this.pbxGroupKeyByName(group);
    file = this.addFile(path, groupKey, opt);
...
pbxProject.prototype.addFrameworkFile = function(proj, fpath, opt) {
    ...
    var frameworkBuildPhase = this.getPBXBuildPhase(proj, 'Frameworks');

    this.addToPbxBuildFileSection(file);        // PBXBuildFile
    this.addToPbxFileReferenceSection(file);    // PBXFileReference

    frameworkBuildPhase.files.push({
        value: file.uuid,
        comment: fpath + ' in Frameworks'
    });
    var group = this.pbxGroupKeyByName(proj);

    this.addToPbxGroup(file, group);

    //this.addFile(path, groupKey, opt);
    // this.addToFrameworksPbxGroup(file);         // PBXGroup
    // this.addToPbxFrameworksBuildPhase(file);    // PBXFrameworksBuildPhase
...
pbxProject.prototype.createPbxGroup = function(name, path, sourceTree) {
    ...
    var existing = this.pbxGroupByName(name);
    if(existing != null) {
        return existing;
    }
...
pbxProject.prototype.clearConfigDefine = function(project, name) {
    var configs = this.pbxBuildConfigurationsByName(project, name);
    if(configs.length == 0) {
        console.log(project, ' configurations were not found');
        return;
    }

    for(var j = 0; j < configs.length; j++) {
        configs[j].buildSettings.GCC_PREPROCESSOR_DEFINITIONS = [];
    }
}

pbxProject.prototype.addConfigDefine = function(project, def, name) {
    var configs = this.pbxBuildConfigurationsByName(project, name);
    if(configs.length == 0) {
        console.log(project, ' configurations were not found');
        return;
    ...
    if(!def) {
        console.log('No define was given');
        return;
    }

    for(var j = 0; j < configs.length; j++) {
        var hasDefine = configs[j].buildSettings.GCC_PREPROCESSOR_DEFINITIONS.indexOf(def) >= 0;
        if(!hasDefine) {
            configs[j].buildSettings.GCC_PREPROCESSOR_DEFINITIONS.push(def);
        }
    }
}
pbxProject.prototype.clearConfigLibraryPath = function(project, name) {
    var configs = this.pbxBuildConfigurationsByName(project, name);
    if(configs.length == 0) {
        console.log(project, ' configurations were not found');
        return;
    ...
    for(var j = 0; j < configs.length; j++) {
        configs[j].buildSettings.LIBRARY_SEARCH_PATHS = [];
    }
}

pbxProject.prototype.addConfigLibraryPath = function(project, path, name) {
    var configs = this.pbxBuildConfigurationsByName(project, name);
    if(configs.length == 0) {
        console.log(project, ' configurations were not found');
        return;
    ...
    if(!path) {
        console.log('No path was given');
        return;
    }

    for(var j = 0; j < configs.length; j++) {
        var hasDefine = configs[j].buildSettings.LIBRARY_SEARCH_PATHS.indexOf(path) >= 0;
        if(!hasDefine) {
            configs[j].buildSettings.LIBRARY_SEARCH_PATHS.push(path);
        }
    }
}

pbxProject.prototype.clearConfigHeaderPath = function(project, name) {
    var configs = this.pbxBuildConfigurationsByName(project, name);
    if(configs.length == 0) {
        console.log(project, ' configurations were not found');
        return;
    ...
    for(var j = 0; j < configs.length; j++) {
        configs[j].buildSettings.USER_HEADER_SEARCH_PATHS = '""';
    }
}

pbxProject.prototype.addConfigHeaderPath = function(project, path, name) {
    var configs = this.pbxBuildConfigurationsByName(project, name);
    if(configs.length == 0) {
        console.log(project, ' configurations were not found');
        return;
    ...
    for(var j = 0; j < configs.length; j++) {
        console.log(configs[j].buildSettings);

        var hasDefine = configs[j].buildSettings.USER_HEADER_SEARCH_PATHS.indexOf(path) >= 0;
        if(!hasDefine) {
            var orig = configs[j].buildSettings.USER_HEADER_SEARCH_PATHS.split('"').join('');

            console.log('CLEANING: ', orig);
            configs[j].buildSettings.USER_HEADER_SEARCH_PATHS = '"' + orig + ' ' + path.split(' ').join('\\ ') + '"';
        }
        console.log(configs[j].buildSettings);
    }
...
pbxProject.prototype.pbxGroupKeyByName = function(name) {
    var groups = this.hash.project.objects['PBXGroup'],
        key, groupKey;
...
pbxProject.prototype.pbxBuildConfigurationsByName = function(project, name) {
    var configs = this.hash.project.objects.XCBuildConfiguration,
        key, configKey;
    ...
    for (key in configs) {
        ...
        configKey = key.split(COMMENT_KEY)[0];
        if(configs[configKey].buildSettings.PRODUCT_BUNDLE_IDENTIFIER == project) {
            // This allows for getting all of the configs or just one (ex: Debug or Release)
            if(!name || (name && configs[key] == name)) {
                result.push(configs[configKey]);
            }
        }
    }
...
pbxProject.prototype.pbxBuildConfigurationKeysByName = function(project, name) {
    var configs = this.hash.project.objects.XCBuildConfiguration,
        key, configKey;
...
pbxProject.prototype.getPBXNativeTarget = function(name) {
    var nativeTargets = this.hash.project.objects['PBXNativeTarget'];
    for(key in nativeTargets) {
        if(nativeTargets[key].name == name) {
            return nativeTargets[key];
        }
    }
}

pbxProject.prototype.getPBXBuildPhase = function(name, target) {
    var nativeTargets = this.hash.project.objects['PBXNativeTarget'];
    for(key in nativeTargets) {
        if(nativeTargets[key].name == name) {
            if(target) {
                for(var i = 0; i < nativeTargets[key].buildPhases.length; i++) {
                    if(nativeTargets[key].buildPhases[i].comment == target) {
                        return this.hash.project.objects.PBXFrameworksBuildPhase[nativeTargets[key].buildPhases[i].value];
                    }
                }
            }
        }
...
pbxProject.prototype.getPBXBuildPhaseKey = function(name, target) {
    var nativeTargets = this.hash.project.objects['PBXNativeTarget'];
    for(key in nativeTargets) {
        if(nativeTargets[key].name == name) {
            if(target) {
                for(var i = 0; i < nativeTargets[key].buildPhases.length; i++) {
                    if(nativeTargets[key].buildPhases[i].comment == target) {
                        return nativeTargets[key].buildPhases[i].value;
                    }
                }
            }
        }
...
pbxProject.prototype.getPBXBuildPhases = function(name, target) {
    var nativeTargets = this.hash.project.objects['PBXNativeTarget'];
    for(key in nativeTargets) {
        if(nativeTargets[key].name == name) {
            if(target) {
                for(var i = 0; i < nativeTargets[key].buildPhases.length; i++) {
                    if(nativeTargets[key].buildPhases[i].comment == target) {
                        return [ nativeTargets[key].buildPhases[i] ];
                    }
                }
            } else {
                return nativeTargets[key].buildPhases;
            }
        }
    }
}
